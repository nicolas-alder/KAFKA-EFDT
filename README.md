...
6. mvn package
7. Open the Jupyter Notebook EFDT_Handling and preprocess the data
8. java -jar target/EFDT-1.0-SNAPSHOT-jar-with-dependencies.jar "\<path to categorized dataset\>" "\<security threshold (e.g. 0.95)\>"
...
If this criterion is true, there are two possibilities. First, <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;X_A" target="_blank"><img src="https://latex.codecogs.com/svg.latex?\inline&space;X_A" title="X_A" /></a> could be the nullsplit which leads to the so called "subtree kill" where the node becomes a reinitialized leaf and its child nodes are deleted. In the other case, the child nodes are deleted again, but the current node is overwritten with the new best attribute instead of the leaf.
...
In this plot one can see that the higher the safety threshold <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;$\delta" target="_blank"><img src="https://latex.codecogs.com/svg.latex?\inline&space;$\delta" title="$\delta" /></a> is set, the later the limit of the Hoeffding criterion converges towards 0. This in turn has the consequence that a deviation appears significant later and thus the following reaction is also performed later (no temporary property is assumed).
...
In summary, it can be stated that a shuffling and a higher amount of data instances could have a positive effect on the accuracy and robustness of the algorithm. In addition, a smaller <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;\delta" target="_blank"><img src="https://latex.codecogs.com/svg.latex?\inline&space;\delta" title="\delta" /></a> for smaller data streams could lead to faster convergence.
